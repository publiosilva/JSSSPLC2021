...
function predictMaintainability(x, root, path = []) {
    const {
    ...
    if (x[feature] <= threshold) return predictMaintainability(x, left, path);

    return predictMaintainability(x, right, path);
}

function evaluateMaintainability(sample) {
    const prediction = predictMaintainability(sample, decisionTree);
    const suggestions = getSuggestionsForImprovement(sample, prediction);

    return { sample, prediction, suggestions };
}
